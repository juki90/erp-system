    ...
    methods: {
        checkForServerFormErrors(error) {
            if (
                ...
                [BAD_REQUEST, UNAUTHORIZED, NOT_FOUND].includes(
                    error.response.status
                )
            ) {
        ...
        checkSameAsPassword(inputName, formName = 'formData') {
            const errors = [];
            let input;
            ...
            !input.sameAsPassword &&
                errors.push('Password repeat does not match password above');
        ...
        checkErrors(inputName, formName = 'formData') {
            ...
            if (input.$params.sameAsPassword) {
                errors = errors.concat(
                    this.checkSameAsPassword(inputName, formName)
                );
            }
